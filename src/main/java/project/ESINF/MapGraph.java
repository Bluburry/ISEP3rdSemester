...
public class MapGraph<V, E> extends CommonGraph<V, E> {
    ...
    public Collection<V> adjVertices(V vert) {
        // get the map of adjacent vertices of vert(has both edges and vertices)
        MapVertex<V, E> mv = mapVertices.get(vert);

        // if vert is not in the graph, return null
        if (mv == null) return null;

        // if it is, return the collection of vertices that are adjacent to vert
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        List<Edge<V, E>> incomingEdgesList = new ArrayList<>();

        // get all vertices of the graph
        List<V> vertices = new ArrayList<>(mapVertices.keySet());

        // for each vertice in the graph
        for(V vertex : vertices){
            // get the edge between the vertice and the vertice we are looking for
            Edge<V, E> edge = edge(vertex, vert);
            // if the edge exists, add it to the list
            if(edge != null){
    ...
    public int addVertexes(Collection<V> vertexes) {
        int numVert = 0;
        for (V vert : vertexes)
            try {
                addVertex(vert);
                ...
                System.out.println("Error inserting vertex " + vert);;
            }
        return numVert;
    }
